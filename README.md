...
이 프로젝트는 Python(Flask) 서버 + Spring Legacy 서버가 통합된 구조입니다.  
교통(기차 / 버스 / 항공 / 결제 등) API 통합 기능을 제공합니다.
...
3. 폴더 구조가 정리된 상태에서 **VSCode**로 드래그앤드롭하여 열기  
...
### 1. 패키지 설치  
VSCode 하단 터미널 또는 PowerShell에서 아래 명령어 순서대로 실행:
